...
tfile <- list.files("data-raw/scout_newcheck", full.names = TRUE)
...
tvalues <- get_station_data(tid, source = "C:/Users/Brennan/Desktop/National_Loads_Master/data/ghcnd_all/")
...
outlier_combined_alt <- outlier_combined_alt %>%
  dplyr::select(ID, DATE, ELEMENT, VALUE, OUTLIER)
...
left_out <- dplyr::anti_join(outlier_combined, scout_outlier,
                             by = c("ID", "DATE", "ELEMENT", "VALUE"))
